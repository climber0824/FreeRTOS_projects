...
static const uint16_t timer_divider = 8;          // Divie 80 MHz
static const uint64_t timer_max_count = 1000000;  // Timer counts
...
// The function executes when timer reaches max (and resets)
...
void doCLI(void *parameters) {
  ...
  while (1) {
    ...
    if (Serial.available() > 0) {
      ...
      // Print new line and check input on 'enter'
      if ((c == '\n') || (c == '\0')) {
        Serial.print("\r\n");
...
void calcAverage(void *parameters) {
  ...
  while (1) {
    ...
    // section is the easiest for the application.
    portENTER_CRITICAL(&spinlock);
    adc_avg = avg;
    portEXIT_CRITICAL(&spinlock);

    // If we took too long to process, buffer writing will overrun.
    // So we send a message to be printed out to the serial terminal.
    if (buf_overrun == 1) {
      strcmp(msg.body, "Error: Buffer overrun. Samples have been dropped.");
...
void setup() {
  ...
  // Start task to handle CLI events. Let's set it at a
